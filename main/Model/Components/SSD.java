...
public class SSD extends CSVUtil{
  ...
  public SSD (int id, String brand, int storageSize, int wattage, double price, boolean status) {
      this.id = id;
      this.brand = brand;
  ...
  public String toString() {
      return "SSD{" + "id=" + id + ", brand='" + brand + '\'' + ", storageSize=" + storageSize + ", wattage=" + wattage +  ", price=" + price + ", status=" + status + '}';
  }

  public static List<SSD> readSSDFromCSV(String fileName) {
          return readFromCSV(fileName, data -> new SSD(Integer.parseInt(data[0]), data[1], Integer.parseInt(data[2]), Integer.parseInt(data[3]), Double.parseDouble(data[4]), Boolean.parseBoolean(data[5])));
  } 
